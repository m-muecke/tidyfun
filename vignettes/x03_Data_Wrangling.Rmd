...
There is a known caveat when calculating the mean of tf vectors within a `data.table` context.
By default, `data.table` will optimize calculations, which may cause `mean` to dispatch to data.table's own optimized implementation,
...
- Set `options(data.table.optimize = 0)`, which disables optimization, allowing the mean function to correctly dispatch to `tidyfun`.
- Alternatively, explicitly call `tf:::mean.tf(activity)` in your code to directly specify the tf mean calculation.

  This caveat applies specifically to `mean()` and is the only known issue when working with `tidyfun` functional data objects within data.table.
